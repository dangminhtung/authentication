...
class QrCode:
	...
	def encode_segments(segs: Sequence[QrSegment], ecl: QrCode.Ecc, minversion: int = 1, maxversion: int = 40, mask: int = -1, boostecl: bool = True) -> QrCode:
		...
		for newecl in (QrCode.Ecc.MEDIUM, QrCode.Ecc.QUARTILE, QrCode.Ecc.HIGH):  # From low to high
			if boostecl and (datausedbits <= QrCode._get_num_data_codewords(version, newecl) * 8):
				ecl = newecl
		print(f"Version: {version}, Mã sửa lỗi: {ecl}")
	...
	def __init__(self, version: int, errcorlvl: QrCode.Ecc, datacodewords: Union[bytes,Sequence[int]], msk: int) -> None:
		...
		# print(''.join(f'{byte:08b}' for byte in datacodewords))
		...
		# print(''.join(f'{byte:08b}' for byte in bytearray(datacodewords)))
		# for byte in bytearray(allcodewords):
		# 	print(byte)
	...
	def changeBit(self,_number):
		if(_number==0) : _number=1
		else : _number=0
		return _number
	def CalEmbedder(self,number,w):
		print("---------------------")
		print(number, format(number, '08b'))
		bit_positions = [1, 3, 5]
		b = [(number >> pos) & 1 for pos in bit_positions]
		print("Các bit tại vị trí 1, 3, 5:", b, "w = ",w)
		d = sum(b[i] ^ w[j] for i,j in zip(range(1,3),range(2)))
		print("d= ", d)
		if( b[0] == 0 ):
			if(d == 1):
				if(b[2] == w[1]): b[1] = w[0]
			...
			if(d == 1):
				if(b[2] == w[1]): b[2] = self.changeBit(w[1])
				else : b[1] = self.changeBit(w[0])
			elif(d == 0): b[0] = 0
		
		# chuyển bit về number ban đầu 
		for i, pos in enumerate(bit_positions):
			if b[i] == 1:
				number |= (1 << pos)  # Đặt bit thành 1
			else:
				number &= ~(1 << pos)  # Đặt bit thành 0

		print("thay doi", b)
		print("sau khi thay doi", number, format(number, '08b'))
		return number
	def EmbedLen(self,binary_data, ecc):
		binary_len= len(binary_data)
		binary_representation = bin(binary_len)[2:]
		w=[-1,-1]
		if(len(binary_representation) == 9 ):
			binary_representation = '0'+ binary_representation
		print(binary_representation)
		k=0;
		for i in range(0,10,2):
			w[0] = int(binary_representation[i])
			w[1] = int(binary_representation[i + 1])
			# print("w =", w)
			ecc[k] = self.CalEmbedder(ecc[k],w)
			k = k+2
		return ecc
	def EmbedData(self,binary_data,ecc):
		binary_len= len(binary_data)
		w=[-1,-1]
		k = 0
		for i in range(0,binary_len,2):
			w[0]= int(binary_data[i])
			w[1]= int(binary_data[i+1])
			ecc[k]=self.CalEmbedder(ecc[k],w)
			k = k+2
		return ecc
	def _add_ecc_and_interleave(self, data: bytearray) -> bytes:
		print(self._binarydata)
		...
		print("Số lượng khối mã lỗi sẽ được tạo ra" ,numblocks)
		print("Số lượng codewords sửa lỗi cần thêm vào mỗi khối.", blockecclen)

		# tính len(w)
		numEmbedData= len(self._binarydata) / numblocks
		...
		# tach chuoi thanh cac chuoi nho hon
		chunks = [self._binarydata[i:i + numEmbedData] for i in range(0, len(self._binarydata), numEmbedData)]
		...
		for i in range(numblocks):
			...
			# thuc hien nhung
			if i==0:
				print(len(self._binarydata))
				ecc = self.EmbedLen(self._binarydata,ecc)
			elif i <= numChunks and i !=0:
				print(f">>>>>>Block {i}: >>>>>>>>>>>>>>>>>")
				ecc = self.EmbedData(chunks[i-1],ecc)
		...
		# 		print(f"Block {index}:" , len(blocks[index]))
		# 		for byte in block:
		# 			print(f"  Byte: {byte} (Binary: {format(byte, '08b')})")  # In byte và dạng nhị phân
		...
		# print(''.join(f'{byte:08b}' for byte in result))
		return result
...
class QrSegment:
	...
	def make_bytes(data: Union[bytes,Sequence[int]]) -> QrSegment:
		...
		for b in data:
			bb.append_bits(b, 8)
		# print("du lieu sau khi chuyen ve byte: " ,data)
		# print(''.join(f'{byte:08b}' for byte in bb))
